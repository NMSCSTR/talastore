    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden md:flex space-x-8 items-center">
                ...
                @auth
                    <a href="/dashboard" class="bg-blue-600 text-white px-5 py-2 rounded-full hover:bg-blue-700 transition">Dashboard</a>
                @else
                    <a href="/login" class="text-gray-600 hover:text-blue-600 transition font-medium">Login</a>
                    <a href="/register" class="bg-blue-600 text-white px-5 py-2 rounded-full hover:bg-blue-700 transition shadow-lg shadow-blue-200">Join Now</a>
                @endauth
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 flex flex-col md:flex-row items-center">
        <div class="md:w-1/2 text-center md:text-left" data-aos="fade-right">
            ...
            <p class="text-lg text-gray-600 mb-8 max-w-lg">
                Shop from local suppliers and high-quality brands all in one place. Fast delivery and secure payments guaranteed.
            </p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <a href="#shop" class="bg-blue-600 text-white px-8 py-4 rounded-xl font-semibold hover:bg-blue-700 transition-all transform hover:scale-105">
                    Start Shopping
                </a>
                <a href="#" class="bg-white border border-gray-200 text-gray-700 px-8 py-4 rounded-xl font-semibold hover:bg-gray-50 transition">
                    Learn More
                </a>
            </div>
        </div>
        <div class="md:w-1/2 mt-12 md:mt-0 relative" data-aos="zoom-in" data-aos-delay="200">
            <img src="https://illustrations.popsy.co/blue/online-shopping.svg" alt="Hero Illustration" class="w-full h-auto">
        </div>
    </div>
</section>
...
<section id="shop" class="py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {{-- This is where your @foreach($products as $product) loop will go --}}
            @for($i = 1; $i <= 4; $i++)
            <div class="bg-white rounded-2xl p-4 shadow-sm hover:shadow-xl transition-all group" data-aos="fade-up" data-aos-delay="{{ $i * 100 }}">
                <div class="h-48 bg-gray-100 rounded-xl mb-4 overflow-hidden relative">
                    <img src="https://via.placeholder.com/300x300?text=Product+{{$i}}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                    <span class="absolute top-2 right-2 bg-white px-2 py-1 rounded-md text-xs font-bold text-blue-600 shadow-sm">New</span>
                </div>
                <h3 class="font-semibold text-gray-800 text-lg">Product Name {{$i}}</h3>
                <p class="text-sm text-gray-500 mb-3">Supplier Name</p>
